...
class shijing
{
    ...
    public function run()
    {
        ...
        //唐诗json文件的路径
        $tangFilePathList = glob("{$sourceFilePath}shijing.json");
        ...
        for ($i = 1; $i <= $this->_section; $i++) {
            file_put_contents(sprintf($sqlPathString, $i), "INSERT INTO `shi_jing` (`shijing_id`, `title`, `chapter`,`section`, `content`,`create_time`) VALUES \r\n");
        }
//dynasty/home/li914/work/chinese-poetry-to-mysql-tool/chinese-poetry-tang-1.sql
        ...
        foreach ($tangFilePathList as $fileCount => $filePath) {
            ...
            foreach ($fileContentArray as $value) {
                //过滤
                if ($this->_ifFilter) {
                    $isAllow = $this->filter($value['content']);
                    if ($isAllow == false) {
                        continue;
                    }
                }

                $paragraphs = implode($value['content'], '\n');
...
//                $content .= "(\"{$uuid}\",\"{$value['title']}\",\"{$value['author']}\",\"T\",\"{$tags}\",\"{$paragraphs}\",\"{$strains}\",{$time})";
                $content .= "(\"{$uuid}\",\"{$value['title']}\",\"{$value['chapter']}\",\"{$value['section']}\",\"{$paragraphs}\",{$time})";
            }

...
class ci_author
{
    ...
    public function run()
    {
        ...
        //唐诗json文件的路径
        $tangFilePathList = glob("{$sourceFilePath}author.song.json");
        ...
        for ($i = 1; $i <= $this->_section; $i++) {
            file_put_contents(sprintf($sqlPathString, $i), "INSERT INTO `ci_author` (`author`, `intro`,`short_intro`,`dynasty`,`create_time`) VALUES \r\n");
        }
...
//                var_dump($value);
                ...
                $time = time();
//                $content .= "(\"{$value['name']}\",\"{$value['description']}\",\"{$value['short_description']}\",\"S\",{$time})";//short_description
                $content .= "('{$value['name']}','{$value['description']}','{$value['short_description']}','S',{$time})";//short_description
            }
